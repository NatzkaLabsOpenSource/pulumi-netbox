...
def get_vlan_group(name: Optional[str] = None,
    ...
    :param str name: At least one of `name`, `slug` or `scope_type` must be given.
    :param int scope_id: Required when `scope_type` is set.
    :param str scope_type: Valid values are `dcim.location`, `dcim.site`, `dcim.sitegroup`, `dcim.region`, `dcim.rack`, `virtualization.cluster` and `virtualization.clustergroup`. At least one of `name`, `slug` or `scope_type` must be given.
    :param str slug: At least one of `name`, `slug` or `scope_type` must be given.
    """
    __args__ = dict()
    __args__['name'] = name
